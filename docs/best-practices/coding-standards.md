...
- **Exception à `final` sur la classe elle-même : tout composant géré par Spring.** Une classe portant une annotation de stéréotype (`@Component`, `@Service`, `@Repository`, `@Controller`, `@RestController`, `@Configuration`) ne se marque jamais `final` — Spring doit pouvoir la sous-classer via un proxy CGLIB dès qu'une préoccupation transverse s'y attache (`@Transactional`, `@Async`, `@Cacheable`, sécurité par méthode, instrumentation Micrometer, retry) ou, pour un `@Configuration`, pour que les appels entre méthodes `@Bean` renvoient le même singleton (`proxyBeanMethods` par défaut à `true`). Rien de tout ça n'est utilisé aujourd'hui dans le projet, mais laisser la classe non-`final` évite par construction un `BeanCreationException` le jour où ça change, plutôt que de devoir le corriger dans l'urgence. Les champs de ces classes restent `private final` : seul le mot-clé sur la classe disparaît.
- **Les classes de `domain`/`application` restent `final`**, y compris les use-cases exposés comme bean via une méthode `@Bean` explicite dans un `*Config` : elles ne portent aucune annotation Spring (ADR 0001) et aucune préoccupation transverse ne leur est appliquée. Si ça change un jour, l'erreur de démarrage sera immédiate et sans ambiguïté — ce n'est pas une raison de les priver de `final` par anticipation.
...
- **Document à forte contention concurrente (agrégat mono-instance, ex : `AlarmSystem`/`home-1`)** : plusieurs threads (scheduler, listener Kafka/événement interne) peuvent légitimement l'écrire au même instant — une collision de verrouillage optimiste (`OptimisticLockingFailureException`) y est attendue, pas exceptionnelle. L'adapter Mongo la traduit en exception de domaine dédiée (ex : `ConcurrentAlarmSystemUpdateException`, pour ne pas faire fuiter de type Spring Data dans `application` — ADR 0001), et le collaborateur `application.service` qui décide de l'écriture relit l'état courant et retente sa décision (nombre borné de tentatives, ex : 5) plutôt que de laisser échouer silencieusement l'appelant. Ne pas retenter en réappliquant aveuglément l'ancienne valeur : il faut rejouer la décision complète sur l'état frais, au cas où l'écriture concurrente ait changé la donne (ex : le système désarmé entre-temps par un humain).
