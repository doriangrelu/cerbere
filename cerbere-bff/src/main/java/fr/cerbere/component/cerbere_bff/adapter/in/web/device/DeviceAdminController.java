...
public final class DeviceAdminController {
	...
	@PostMapping("/devices")
	public String register(@RequestParam final String candidateId,
							@RequestParam(required = false) final String label,
							@RequestParam(required = false) final String zoneId,
							final Model model) {
		final List<SimulatedDeviceResponse> candidates = this.deviceMockClient.listAll();
		final SimulatedDeviceResponse candidate = candidates.stream()
			.filter(simulated -> simulated.id().equals(candidateId))
			.findFirst()
			.orElse(null);
		if (candidate == null) {
			model.addAttribute(DEVICE_ERROR_ATTRIBUTE, "Device sélectionné introuvable ou déjà enregistré.");
			this.populateModel(model);
		...
		final String resolvedLabel = (label == null || label.isBlank()) ? candidate.label() : label;
		this.deviceCoreClient.register(new RegisterDeviceRequest(candidateId, candidate.type(), resolvedLabel, this.blankToNull(zoneId)));
		this.populateModel(model);
		return DEVICE_SECTION_FRAGMENT;
	}
