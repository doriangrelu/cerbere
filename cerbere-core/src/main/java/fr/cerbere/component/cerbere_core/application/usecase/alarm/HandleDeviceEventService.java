...
/**
 * Évalue chaque événement de device rapporté : ignoré si le système est
 * désarmé, si le device est inconnu/désactivé, ou si l'événement ne constitue
 * pas une violation compte tenu du mode d'armement (voir {@link fr.cerbere.component.cerbere_core.domain.model.ArmingMode}
...
public final class HandleDeviceEventService implements HandleDeviceEventUseCase {
	...
	public void handle(final DeviceEventReport report) {
		...
		if (!this.isViolation(alarmSystem.getMode(), report)) {
			return;
		}
